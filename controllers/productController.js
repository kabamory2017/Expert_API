...
exports.getAllProducts = async(req,res)=>{
    try {
        const all_datas = await Product.find().populate({path: 'category',select:'_id name'}).populate({path: 'user',select:'_id name email'})
...
exports.getProductsWithout = async(req,res)=>{
    try {
        const all_datas = await Product.find().select('_id name price').populate({path: 'user',select:'_id name email'})
        res.status(200).json({all_data:all_datas})
    } catch (error) {
        return res.status(500).json({message: error})
    }
...
exports.getProductsByUser = async(req,res)=>{
    ...
    // }
    try {
        const all_datas = await Product.find({user:req.params.id}).select('_id name price').populate({path: 'category',select:'_id name'}).populate({path: 'user',select:'_id name email'})
        res.status(200).json({all_data:all_datas})
    } catch (error) {
        return res.status(500).json({message: error})
    }
...
exports.findProductsById = async(req,res)=>{
    try {
        ...
        const all_datas = await Product.findById(req.params.id).populate({path: 'category',select:'_id name'})
        if(!all_datas){
...
exports.updateProductsById= async(req,res)=>{
    ...
    try {
        ...
        const all_datas = await Product.findByIdAndUpdate(req.params.id, req.body,{new:true})
